...
const PRIORITY_META: Record<string, { classes: string; icon: React.ReactNode }> = {
  High: { classes: "bg-red-50/80 text-red-700 ring-red-600/20", icon: <ArrowUp size={12} /> },
  Medium: { classes: "bg-amber-50/80 text-amber-700 ring-amber-600/20", icon: <ArrowRight size={12} /> },
  Low: { classes: "bg-slate-100/80 text-slate-600 ring-slate-500/20", icon: <ArrowDown size={12} /> },
};
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => {
        ...
        return (
          <GlassCard key={item.id} className={`p-4 flex flex-col gap-2.5 transition-opacity ${done ? "opacity-60" : ""}`}>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-500">
              ...
              <select
                ...
                className="ml-auto border border-white/60 rounded-lg text-xs px-2 py-1 text-slate-700 bg-white/50 backdrop-blur-xl focus:outline-none focus:ring-2 focus:ring-brand-300"
              >
                {STATUSES.map((s) => (
